@extends('page')

@section('title', ' | Budgety')

@section('content')
<div class="row">
    <div class="offset-by-one ten columns">
        <img src="/images/budgety/budgety.png">
        <hr>
        <p>Budgety is a JavaScript budget app, which as I made as a practice project while learning JavaScript.</p>
        <p>You can use the app here:<a href="http://budgety.nicholaskearney.com" target="_blank"><b>http://budgety.nicholaskearney.com</b></a></p>
        <p>You can view the app on GitHub here: <a href="https://github.com/nkear003/budget-app" target="_blank"><b>https://github.com/nkear003/budget-app</b></a></p>
    </div>
</div>
@endsection